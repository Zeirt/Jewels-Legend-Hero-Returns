...
//Analiza las properties de la tarjeta grafica para devolver el tamaño adecuado de tile, tambien trata el tamaño del table
...
__global__ void automaticTableAnalysisKernel(float *table_d, float *aux_d, int difficulty, int width, int height, int TILE_WIDTH) {
	...
	//printf("%i-%f ", tx + ty*width, aux_d[tx + ty*width]);
}
...
void automaticTableAnalysis(int difficulty, float* table, int width, int height, int TILE_WIDTH, curandState* globalState) {
	...
	if (valor_mejor >= 3) {
		swapSpots(table, x_mejor, y_mejor, 4, width, height, 1, difficulty, TILE_WIDTH, globalState);
	}
	free(aux);
	free(erasedJewels);
	cudaFree(table_d);
